                ...
                @if(!empty($fcm_credentials))
                    ...
                    function startFCM() {
                        messaging
                            ...
                            .then(function(token) {
                                subscribeTokenToBackend(token, 'goya_provider_{{auth()->user()->provider->zone_id}}_{{ auth()->user()->provider->id }}_booking_message');
                                @if($serviceAtProviderPlace)
                                    @if(in_array('customer', $serviceLocations))
                                        subscribeTokenToBackend(token, 'goya_provider_{{auth()->user()->provider->zone_id}}_customer_booking_message');
                                   @endif
                                   @if(in_array('provider', $serviceLocations))
                                        subscribeTokenToBackend(token, 'goya_provider_{{auth()->user()->provider->zone_id}}_provider_booking_message');
                                   @endif
                                @else
                                   subscribeTokenToBackend(token, 'goya_provider_{{auth()->user()->provider->zone_id}}_booking_message');
                                @endif
